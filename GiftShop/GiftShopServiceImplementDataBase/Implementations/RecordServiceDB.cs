    ...
    public class RecordServiceDB : IRecordService
    {
        ...
        public void SaveSetPrice(RecordBindingModel model)
        {
            ...
            try
            {
                ...
                var table = document.Tables.Add(rangeTable, sets.Count, 2, ref missing, ref missing);
                font = table.Range.Font; font.Size = 14;
                font.Name = "Times New Roman";
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = sets[i].SetName;
                    table.Cell(i + 1, 2).Range.Text = sets[i].Price.ToString();
                }
        ...
        public List<StoragesLoadViewModel> GetStoragesLoad()
        {
            return context.Storages.ToList().GroupJoin(
                 ...
                 new StoragesLoadViewModel
                 {
                     ...
                     Parts = storageCompList.Select(r =>
                     new Tuple<string, int>(r.Part.PartName,
                     r.Count))
                 }).ToList();
        }
        ...
        public void SaveCustomerProcedures(RecordBindingModel model)
        {
            ...
            //заполняем таблицу      
            var list = GetCustomerProcedures(model);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].SetName, fontForCells));
                table.AddCell(cell);
